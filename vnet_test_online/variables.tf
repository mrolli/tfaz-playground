...
variable "resource_name_templates" {
  ...
  default = {
    resource_group_name         = "rg-$${workload}-$${environment}-$${location}-$${uniqueness}"
    virtual_network_name        = "vnet-$${workload}-$${environment}-$${location}-$${uniqueness}"
    network_security_group_name = "nsg-$${workload}-$${environment}-$${location}-$${uniqueness}"
  }
}
